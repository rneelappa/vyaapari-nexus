...
async function fixVoucherRelationships(supabase: any, companyId: string, divisionId: string) {
  ...
  // Fix trn_accounting relationships
  ...
  if (accError) {
  ...
  } else if (accountingRows) {
    results.accounting.total = accountingRows.length;
    console.log(`[Fix] Found ${accountingRows.length} accounting entries to process`);
    ...
    for (const row of accountingRows) {
      try {
        ...
        if (row.voucher_guid !== voucher.guid) {
  ...
  // Fix trn_inventory relationships
  ...
  if (invError) {
    ...
    results.inventory.total = inventoryRows.length;
    console.log(`[Fix] Found ${inventoryRows.length} inventory entries to process`);
