...
/**
 * Test Tally GET API Endpoints - V2 Production with Correct API Key
...
// Test 12: Test with New Company/Division IDs
async function testWithNewIds() {
  ...
  try {
    const response = await makeRequest(
      `${API_BASE_URL}/tally-get-ledgers?company_id=${newCompanyId}&division_id=${newDivisionId}`
    );
    console.log(`  Status: ${response.statusCode}`);
...
async function runGetApiTests() {
  console.log('🚀 Testing Tally GET API Endpoints - V2 Production with Correct API Key');
  console.log('=======================================================================');
