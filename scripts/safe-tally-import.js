...
const TALLY_SCHEMA = 'tally_data'; // Separate schema for Tally data

async function createTallySchema() {
  try {
    console.log('🔧 Creating separate Tally schema...');
    
    // Create schema if it doesn't exist
    await pool.query(`CREATE SCHEMA IF NOT EXISTS ${TALLY_SCHEMA}`);
    
    // Create Tally tables in the separate schema
    const createTablesSQL = `
      -- Groups table
      CREATE TABLE IF NOT EXISTS ${TALLY_SCHEMA}.mst_group (
      ...
      -- Ledgers table
      CREATE TABLE IF NOT EXISTS ${TALLY_SCHEMA}.mst_ledger (
        guid VARCHAR(64) PRIMARY KEY,
        company_id VARCHAR(36) NOT NULL,
        division_id VARCHAR(36) NOT NULL,
      ...
      -- Stock Items table
      CREATE TABLE IF NOT EXISTS ${TALLY_SCHEMA}.mst_stock_item (
        guid VARCHAR(64) PRIMARY KEY,
        company_id VARCHAR(36) NOT NULL,
        division_id VARCHAR(36) NOT NULL,
        name VARCHAR(1024) NOT NULL,
      ...
      -- Vouchers table
      CREATE TABLE IF NOT EXISTS ${TALLY_SCHEMA}.trn_voucher (
        guid VARCHAR(64) PRIMARY KEY,
        company_id VARCHAR(36) NOT NULL,
        division_id VARCHAR(36) NOT NULL,
    ...
    console.log('✅ Tally schema and tables created safely');
    
  } catch (error) {
    console.error('❌ Error creating Tally schema:', error.message);
    throw error;
  }
}
...
async function insertSampleData() {
  try {
    ...
    await insertDataSafely(`${TALLY_SCHEMA}.mst_group`, groups);
    await insertDataSafely(`${TALLY_SCHEMA}.mst_ledger`, ledgers);
    await insertDataSafely(`${TALLY_SCHEMA}.mst_stock_item`, stockItems);
    await insertDataSafely(`${TALLY_SCHEMA}.trn_voucher`, vouchers);
...
async function verifyData() {
  try {
    ...
    const tables = [
      `${TALLY_SCHEMA}.mst_group`,
      `${TALLY_SCHEMA}.mst_ledger`, 
      `${TALLY_SCHEMA}.mst_stock_item`,
      `${TALLY_SCHEMA}.trn_voucher`
    ];
...
async function main() {
  ...
  console.log(`📊 Using separate schema: ${TALLY_SCHEMA}`);
  ...
  try {
    ...
    // Create separate schema and tables
    await createTallySchema();
    ...
    console.log(`✅ Tally data stored in separate schema: ${TALLY_SCHEMA}`);
