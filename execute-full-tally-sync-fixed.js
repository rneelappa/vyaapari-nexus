...
// Function to parse XML data with proper numeric handling
function parseXmlData(xmlData, recordTag, fields) {
  ...
  for (const match of matches) {
    ...
    for (const field of fields) {
      ...
      let value = fieldMatch ? fieldMatch[1] : (field.defaultValue || '');
      
      // Handle numeric fields
      if (field.isNumeric) {
        value = parseNumeric(value, field.defaultValue || 0);
      }
...
async function executeFullTallySync() {
  console.log('🚀 Starting Full Tally Sync - Fixed Version');
  console.log('============================================');
  ...
  try {
    ...
    if (ledgerXml) {
      const ledgerFields = [
        ...
        { xmlTag: 'OPENINGBALANCE', dbField: 'opening_balance', defaultValue: 0, isNumeric: true },
        { xmlTag: 'CLOSINGBALANCE', dbField: 'closing_balance', defaultValue: 0, isNumeric: true },
